class BTreeNode {
    public maxKeys: number;       // Максимальное количество ключей в узле
    public isLeaf: boolean;       // Показывает, является ли узел листом
    public keys: number[];        // Список ключей в узле
    public children: BTreeNode[]; // Список дочерних узлов
    public minDegree: number;     // Минимальная степень (T)

    constructor(maxKeys: number, isLeaf: boolean = true) {
        this.maxKeys = maxKeys;
        this.isLeaf = isLeaf;
        this.keys = [];
        this.children = [];

        // Узел может содержать максимум 'maxKeys' ключей,
     ...
     */
    public insertNonFull(key: number): void {
     ...
     */
    public splitChild(index: number): void {
    ...
    public deleteKey(key: number): void {
        ...
        if (idx < this.keys.length && this.keys[idx] === key) {
            ...
            if (this.isLeaf) {
                // Если узел лист, то значит ключа в дереве нет
                return;
            }
     ...
     */
    private getPredecessor(idx: number): number {
     ...
     */
    private getSuccessor(idx: number): number {
    ...
    private borrowFromPrev(idx: number): void {
        ...
        this.keys[idx - 1] = sibling.keys.pop() as number;
    ...
    private borrowFromNext(idx: number): void {
        ...
        this.keys[idx] = sibling.keys.shift() as number;
...
export default class BTree {
    ...
    public insert(key: number): void {
        if (!this.root) {
            // На случай, если root был обнулён
            this.root = new BTreeNode(this.maxKeys, true);
        }
